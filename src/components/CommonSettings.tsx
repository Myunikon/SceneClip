...
interface OptionCardProps {
    ...
    activeColor?: 'red' | 'orange' | 'purple' | 'emerald' | 'blue'
    disabled?: boolean
...
export function OptionCard({
    icon, title, description, activeColor = 'blue', disabled = false, disabledReason,
...
export function SettingCard({
    ...
    activeColor = 'red', disabled, disabledReason, expandableContent, onClick, className
}: SettingCardProps) {
    const colorClasses = {
...
export function ChoiceGroup<T extends string | number | undefined>({
    ...
    // Default GRID & SCROLL Variants
    return (
        <div className={cn(
            variant === 'grid' ? cn("grid gap-2", gridClasses) : "flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar"
        )}>
            {options.map((opt) => {
                const isSelected = value === opt.value
                const Icon = opt.icon
                ...
                return (
                    <button
                        ...
                        className={cn(
                            ...
                            variant === 'scroll' ? "flex-none min-w-[5rem]" : "min-h-[42px]",
                            isSelected
