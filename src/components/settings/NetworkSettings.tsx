    ...
    return (
        <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
            <section className="p-5 border rounded-xl bg-card/30 space-y-4">
                ...
                <div className="space-y-2 pt-2">
                    <label className="text-xs font-semibold uppercase text-muted-foreground flex items-center gap-2">
                        <Globe className="w-3 h-3"/> {t.settings.network?.user_agent || "User Agent"}
                    </label>
                    <input 
                        className="w-full p-2 rounded-md border bg-background/50 font-mono text-xs focus:ring-1 focus:ring-primary outline-none"
                ...
                <div className="space-y-4 pt-2">
                    <div className="flex flex-col space-y-2">
                        <label className="text-xs font-semibold uppercase text-muted-foreground flex items-center justify-between">
                            <span className="flex items-center gap-2"><Wifi className="w-3 h-3 text-blue-400"/> {t.settings.network?.concurrent_fragments || "Concurrent Fragments"}</span>
                        ...
                        <div className="bg-secondary/20 p-4 rounded-xl space-y-4 border border-border/50">
                            ...
                            <div className="text-[10px] text-muted-foreground space-y-2 border-t border-border/50 pt-3">
                                ...
                                <ul className="list-disc pl-4 space-y-1 opacity-80">
                                    <li><strong>1-4 (Safe):</strong> Best for general usage and stable downloads.</li>
                                    <li><strong>5-8 (Fast):</strong> Recommended for high-speed connections. Increases CPU usage.</li>
                                    <li><strong>9-16 (Aggressive):</strong> Maximum throughput. May cause temporary IP band (429 Too Many Requests) if abused.</li>
                                </ul>
                                <p className="text-red-400/80 italic mt-1">
                                    * Applies to all download modes (Full Video, Audio, & Trim).
                                </p>
                            </div>
                        </div>
                    </div>
