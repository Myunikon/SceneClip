...
import { SquareTerminal, Copy, Check, Trash2, ArrowDown } from 'lucide-react'
...
export function TerminalOutputModal({
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={t('common.view_terminal', 'Live Terminal')}
            className="max-w-4xl"
            headerIcon={
        ...
        >
            {/* Task Info Bar */}
            <div className="px-4 py-2 bg-gray-50/80 dark:bg-secondary/40 border-b border-border flex items-center justify-between gap-4 shrink-0">
                <div className="flex items-center gap-2 min-w-0">
                    {['downloading', 'processing', 'fetching_info', 'queued'].includes(task.status) && (
                        <span className="text-[11px] font-medium bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-500/20 flex items-center gap-1.5 shrink-0">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                            Live
                        </span>
                    )}
                    <span className="text-gray-800 dark:text-gray-400 font-medium text-xs font-mono truncate">{task.title || 'Unknown'}</span>
                    <span className="text-gray-400 dark:text-gray-600 text-xs font-mono shrink-0">•</span>
                    <span className="text-gray-500 dark:text-gray-600 text-xs font-mono select-all truncate">{task.url}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                        ...
                        className="text-xs font-medium flex items-center gap-1 px-2.5 py-1 bg-secondary/80 hover:bg-secondary rounded-lg disabled:opacity-30 transition-colors text-foreground"
                    >
                        {copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                        <span>{copied ? t('terminal.copied', 'Copied!') : t('terminal.copy_all', 'Copy All')}</span>
                    </button>
                    {/* Clear */}
                    <button
                        onClick={clearLines}
                    ...
                    >
                        ...
                        <span>{t('terminal.clear_all', 'Clear')}</span>
                    </button>
                </div>
            </div>
